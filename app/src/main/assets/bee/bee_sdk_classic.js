...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        // XHR works on file:// in Android WebView; fetch of file:// is blocked
        module_or_path = new Promise(function(resolve, reject) {
            var xhr = new XMLHttpRequest();
            xhr.open('GET', 'file:///android_asset/bee/bee_sdk_bg.wasm', true);
            xhr.responseType = 'arraybuffer';
            xhr.onload = function() { resolve(xhr.response); };
            xhr.onerror = function() { reject(new Error('XHR WASM load failed')); };
            xhr.send();
        });
    }
    const imports = __wbg_get_imports();
...
// Expose as window.BeeSDK
window.BeeSDK = {
    init: function(wasmPath) { return __wbg_init(wasmPath); },
    Miner: Miner,
    gen_mining_keys: gen_mining_keys,
    ensure_mining_keys_propagated: ensure_mining_keys_propagated,
    get_miner_address_by_wallet_name: get_miner_address_by_wallet_name,
    MinerAccountData: MinerAccountData,
    ResultOfGenMiningKeys: ResultOfGenMiningKeys,
};
